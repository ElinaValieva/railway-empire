new Vue({
    ...
    methods: {
        ...
        addStationBtn: function () {
            ...
            if (this.stationName && !/^[0-9]+$/.test(this.stationName)) {
                if (!this.latitude || !this.longitude) {
                    this.getCoordinates(this.stationName)
                }
                else {
                    if (/^[0-9]+$/.test(this.longitude) && /^[0-9]+$/.test(this.latitude))
                        this.addStation(this.latitude, this.longitude);
                    else swal({title: 'Oops..', text: 'Wrong values for coordinates..', type: 'error'});
                }
            } else swal({title: 'Oops..', text: 'Wrong station name..', type: 'error'});
        },
        addScheduleBtn() {
            this.addSchedule();
        },
